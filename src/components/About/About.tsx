...
const About = ({ educations, certifications }: { educations: SanitizedEducation[], certifications: SanitizedCertification[] }) => {
  return (
    <section id="about" className="relative overflow-hidden w-full border-t border-[rgba(255,255,255,0.02)] bg-[rgba(10,10,10,0.3)]">
      <div className="diamond-grid-overlay"></div>

      <div className="max-w-[1200px] mx-auto px-[1.5rem] md:px-[3rem] py-[2.8rem] relative z-20">
          ...
          <div className="reveal flex flex-col gap-[1rem]">
            ...
            <div className="achv-stack flex flex-col gap-[0.6rem]">
              {certifications.map((cert, i) => (
                <div key={i} className="achv-card reveal flex items-start gap-[0.85rem] p-[1rem] border border-[var(--border)] bg-[rgba(17,17,17,0.6)] backdrop-blur-[2px] transition-all duration-200 relative overflow-hidden group hover:border-[rgba(232,255,107,0.2)] hover:bg-[var(--surface)]">
                  ...
                  <div className="achv-body flex-1">
                    <div className="achv-title flex justify-between items-start gap-[1rem] mb-[0.15rem]">
                      <span className="text-[0.88rem] font-bold text-[var(--text)] tracking-[-0.01em]">{cert.name}</span>
                      {cert.year && <span className="text-[var(--accent)] font-[var(--mono)] text-[0.6rem] tracking-[0.05em] whitespace-nowrap mt-[0.2rem]">{cert.year}</span>}
                    </div>
